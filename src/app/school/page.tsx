...
export default function SchoolDashboardPage() {
  ...
  const handleGenerateStudent = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setSuccessMessage("Student account generated successfully!");
    ...
    <DashboardLayout>
      <div className="flex flex-col gap-6 w-full max-w-5xl mx-auto py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="glass-panel p-6 border border-white/5 md:col-span-1 h-fit">
            ...
            <div className="w-full bg-white/5 rounded-full h-2 mb-2 border border-white/5">
              <div 
                className={`h-2 rounded-full transition-all duration-500 ${progressPercent >= 100 ? 'bg-red-500' : 'bg-primary'}`}
            ...
            <button 
              ...
              className="w-full mt-6 py-3 bg-primary hover:bg-primary/90 text-primary-foreground font-bold rounded-xl transition-all shadow-[0_0_20px_rgba(255,255,255,0.2)] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {isGenerating ? (
      ...
      {successMessage && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[100] animate-in fade-in slide-in-from-bottom-4 duration-300">
          <div className="glass-panel bg-primary text-primary-foreground px-6 py-3 rounded-2xl shadow-[0_0_40px_rgba(255,255,255,0.3)] flex items-center gap-3 border border-white/20">
