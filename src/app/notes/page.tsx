  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        if (userId) {
          logUserActivity(userId, "Notes", `Analyzed: ${file.name}`);
        }
      };
      reader.onerror = () => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-4">
          ...
          <div className="flex flex-col gap-4">
            ...
            <button
              ...
              className="w-full py-4 bg-primary text-primary-foreground font-medium rounded-xl hover:bg-primary/90 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(79,70,229,0.3)]"
            >
              {isProcessing ? (
