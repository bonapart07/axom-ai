...
import { Users, Shield, Zap, BookOpen, Search, RefreshCw, Trash2, Edit2, X, Check, Save, RotateCcw, MapPin } from "lucide-react";
    ...
    <div className="space-y-6 relative">
      ...
      <div className="glass-panel p-6 border border-white/5">
        <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
          <MapPin className="w-5 h-5 text-primary" /> Traffic by District (Assam)
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
          {Object.entries(
            users.reduce((acc: any, user) => {
              const d = user.district || "Unspecified";
              acc[d] = (acc[d] || 0) + 1;
              return acc;
            }, {})
          )
          .sort((a: any, b: any) => b[1] - a[1])
          .map(([district, count]: any) => (
            <div key={district} className="bg-white/5 border border-white/10 rounded-xl p-3 flex flex-col items-center justify-center text-center">
              <span className="text-xs text-slate-500 truncate w-full px-1">{district}</span>
              <span className="text-lg font-bold text-primary">{count}</span>
            </div>
          ))}
          {users.length === 0 && (
            <p className="text-slate-500 text-sm col-span-full py-4 text-center">Waiting for users to provide district info...</p>
          )}
        </div>
      </div>
          ...
          <div className="relative w-full max-w-lg p-6 overflow-hidden bg-slate-900 border border-primary/30 rounded-2xl shadow-[0_0_40px_rgba(79,70,229,0.2)]">
            ...
            <div className="mt-8 flex gap-3">
              ...
              <button 
                ...
                disabled={isSaving}
                className="flex-1 py-2.5 bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl transition-colors font-medium text-sm flex items-center justify-center gap-2"
              >
